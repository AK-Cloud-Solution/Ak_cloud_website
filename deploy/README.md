...
For Azure login, configure repository secrets named exactly `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET`, `AZURE_TENANT_ID`, and `AZURE_SUBSCRIPTION_ID`. `AZURE_CLIENT_ID` must be the service principal application ID; `AZURE_CLIENT_SECRET` is the matching secret. Grant the service principal `AcrPush` on the Azure Container Registry and sufficient Terraform permissions on the deployment resources.